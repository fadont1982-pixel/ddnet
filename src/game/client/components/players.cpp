	...
	float Intra)
{
	...
	if(ClientId >= 0 && ClientId < MAX_CLIENTS)
	{
		...
		if(str_comp(pClan, "AI-Ghost") == 0)
			Alpha = 0.08f;
		else if(str_comp(pClan, "AI-Leader") == 0)
			Alpha = 1.0f;
	}
	...
	float Intra)
{
	...
	{
		const char *pClan = GameClient()->m_aClients[ClientId].m_aClan;
		if(str_comp(pClan, "AI-Leader") == 0)
		{
			Alpha = 1.0f;
			RenderInfo.m_CustomColoredSkin = true;
			RenderInfo.m_ColorBody = ColorRGBA(1.0f, 0.85f, 0.1f, 1.0f);
			RenderInfo.m_ColorFeet = ColorRGBA(0.9f, 0.7f, 0.0f, 1.0f);
		}
		else if(str_comp(pClan, "AI-Ghost") == 0)
		{
			Alpha = 0.20f;
		}
	}
