...
void CCamera::OnRender()
{
	...
	if(g_Config.m_ClAiCamFollow)
	{
		float LeadX = -999999.0f;
		vec2 LeadPos = vec2(0, 0);
		bool FoundLead = false;
		for(int c = 0; c < MAX_CLIENTS; c++)
		{
			if(GameClient()->m_Snap.m_aCharacters[c].m_Active)
			{
				const char *pClan = GameClient()->m_aClients[c].m_aClan;
				const char *pName = GameClient()->m_aClients[c].m_aName;
				if(str_comp(pClan, "AI-Leader") == 0 || str_comp(pClan, "AI-Ghost") == 0 || str_find(pName, "Bot ") != nullptr)
				{
					vec2 Pos = GameClient()->m_aClients[c].m_RenderPos;
					if(Pos.x > LeadX)
					{
						LeadX = Pos.x;
						LeadPos = Pos;
		...
		if(FoundLead)
		{
			m_Center = mix(m_Center, LeadPos, 0.12f);
		}
	}
