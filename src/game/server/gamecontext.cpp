	...
	m_VoteMutes("votemutes")
{
	...
	m_pEvolutionManager = new CEvolutionManager();
	m_pEvolutionManager->Init(this);
...
CGameContext::~CGameContext()
{
	...
	delete m_pEvolutionManager;
	m_pEvolutionManager = nullptr;
}
...
void CGameContext::OnTick()
{
	...
	*m_World.GetTuning(0) = m_aTuningList[0];
	...
	if(m_pEvolutionManager && m_pEvolutionManager->IsActive())
	{
		m_pEvolutionManager->OnTick();
	}
...
void CGameContext::ConAiStart(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int PopSize = pResult->NumArguments() > 0 ? pResult->GetInteger(0) : 16;
	if(pSelf->m_pEvolutionManager)
	{
		pSelf->m_pEvolutionManager->StartEvolution(PopSize);
		char aBuf[128];
		str_format(aBuf, sizeof(aBuf), "Started AI evolution with %d bots!", PopSize);
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "ai", aBuf);
		pSelf->SendChat(-1, TEAM_ALL, aBuf);
	}
}

void CGameContext::ConAiStop(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	if(pSelf->m_pEvolutionManager)
	{
		pSelf->m_pEvolutionManager->StopEvolution();
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "ai", "Stopped AI evolution.");
		pSelf->SendChat(-1, TEAM_ALL, "Stopped AI evolution.");
	}
}

void CGameContext::ConAiTimescale(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int Scale = pResult->GetInteger(0);
	Scale = std::clamp(Scale, 1, 100);
	pSelf->Config()->m_SvAiTimescale = Scale;
	char aBuf[128];
	str_format(aBuf, sizeof(aBuf), "AI Timescale set to %dx", Scale);
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "ai", aBuf);
	pSelf->SendChat(-1, TEAM_ALL, aBuf);
}

void CGameContext::ConAiTurbo(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int Turbo = pResult->NumArguments() > 0 ? pResult->GetInteger(0) : (pSelf->Config()->m_SvAiTurbo ^ 1);
	pSelf->Config()->m_SvAiTurbo = Turbo ? 1 : 0;
	char aBuf[128];
	str_format(aBuf, sizeof(aBuf), "AI Turbo mode: %s", Turbo ? "ENABLED (MAX speed simulation)" : "DISABLED (Normal simulation)");
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "ai", aBuf);
	pSelf->SendChat(-1, TEAM_ALL, aBuf);
}

void CGameContext::ConAiReset(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	if(pSelf->m_pEvolutionManager)
	{
		pSelf->m_pEvolutionManager->ResetGeneration();
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "ai", "Reset current AI generation.");
	}
}

void CGameContext::ConAiSave(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	const char *pFile = pResult->NumArguments() > 0 ? pResult->GetString(0) : "best_bot.ai";
	if(pSelf->m_pEvolutionManager)
	{
		bool Ok = pSelf->m_pEvolutionManager->SaveBest(pFile);
		char aBuf[128];
		str_format(aBuf, sizeof(aBuf), Ok ? "Saved best AI model to '%s'" : "Failed to save AI model to '%s'", pFile);
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "ai", aBuf);
	}
}

void CGameContext::ConAiLoad(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	const char *pFile = pResult->GetString(0);
	if(pSelf->m_pEvolutionManager)
	{
		bool Ok = pSelf->m_pEvolutionManager->LoadBest(pFile);
		char aBuf[128];
		str_format(aBuf, sizeof(aBuf), Ok ? "Loaded best AI model from '%s'" : "Failed to load AI model from '%s'", pFile);
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "ai", aBuf);
	}
}
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("ai_start", "?i[population]", CFGFLAG_SERVER | CFGFLAG_CHAT, ConAiStart, this, "Start AI neuroevolution with population size (default 16)");
	Console()->Register("ai_stop", "", CFGFLAG_SERVER | CFGFLAG_CHAT, ConAiStop, this, "Stop AI neuroevolution");
	Console()->Register("ai_reset", "", CFGFLAG_SERVER | CFGFLAG_CHAT, ConAiReset, this, "Reset current AI generation");
	Console()->Register("ai_save", "?r[file]", CFGFLAG_SERVER | CFGFLAG_CHAT, ConAiSave, this, "Save best AI neural net to file");
	Console()->Register("ai_load", "r[file]", CFGFLAG_SERVER | CFGFLAG_CHAT, ConAiLoad, this, "Load AI neural net from file");
	Console()->Register("ai_timescale", "i[scale]", CFGFLAG_SERVER | CFGFLAG_CHAT, ConAiTimescale, this, "Set AI timescale simulation speed multiplier (1 to 100)");
	Console()->Register("ai_turbo", "?i[0|1]", CFGFLAG_SERVER | CFGFLAG_CHAT, ConAiTurbo, this, "Toggle AI Turbo fast-training headless mode");
...
void CGameContext::OnInit(const void *pPersistentData)
{
	...
	if(m_pEvolutionManager && !m_pEvolutionManager->IsActive())
	{
		m_pEvolutionManager->StartEvolution(16);
	}
}
