...
void CPlayer::Tick()
{
	...
	if(!GameServer()->m_pController->IsGamePaused())
	{
		...
		bool IsAiBot = (GameServer()->EvolutionManager() && GameServer()->EvolutionManager()->IsActive() && GameServer()->EvolutionManager()->IsBot(m_ClientId));
		if(!m_pCharacter && RespawnTick <= Server()->Tick() && !IsAiBot)
			m_Spawning = true;
...
void CPlayer::ResetForAiRespawn()
{
	if(m_pCharacter)
	{
		delete m_pCharacter;
