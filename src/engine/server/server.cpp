...
{
	int Scale = std::clamp(Config()->m_SvAiTimescale, 1, 100);
	return time_get() + ((int64_t)(Tick - m_CurrentGameTick) * time_freq()) / (TickSpeed() * Scale);
}
...
int CServer::SendMsg(CMsgPacker *pMsg, int Flags, int ClientId)
{
	...
	if(ClientId < 0)
	{
		...
		if(!(Flags & MSGFLAG_NOSEND))
		{
			for(int i = 0; i < MAX_CLIENTS; i++)
			{
				if(m_aClients[i].m_State == CClient::STATE_INGAME && !m_aClients[i].m_DebugDummy)
				{
...
bool CServer::InitBotClient(int ClientId, const char *pName)
{
	...
	// Fake IPv6 address so net callbacks work
	Client.m_DebugDummyAddr.type = NETTYPE_IPV6;
	Client.m_DebugDummyAddr.ip[0] = 0xfd;
	secure_random_fill(&Client.m_DebugDummyAddr.ip[1], 5);
	Client.m_DebugDummyAddr.ip[6] = 0xc0;
	...
	Client.m_DebugDummyAddr.ip[11] = 0x00;
	uint_to_bytes_be(&Client.m_DebugDummyAddr.ip[12], ClientId);
	Client.m_DebugDummyAddr.port = 8303;
	net_addr_str(&Client.m_DebugDummyAddr, Client.m_aDebugDummyAddrString.data(), Client.m_aDebugDummyAddrString.size(), true);
	net_addr_str(&Client.m_DebugDummyAddr, Client.m_aDebugDummyAddrStringNoPort.data(), Client.m_aDebugDummyAddrStringNoPort.size(), false);
...
void CServer::DropBotClient(int ClientId)
{
	if(ClientId < 0 || ClientId >= MaxClients())
		return;
	if(m_aClients[ClientId].m_DebugDummy)
	{
		DelClientCallback(ClientId, "Dropping AI bot", this);
	}
}
...
int CServer::Run()
{
	...
	// start game
	{
		...
		int64_t VirtualTimeAccumulator = 0;
		int64_t LastLoopTime = time_get();
		int64_t LastSnapWallTime = 0;
		...
		while(m_RunServer < STOPPING)
		{
			...
			int64_t RealDt = CurrentTime - LastLoopTime;
			LastLoopTime = CurrentTime;
			if(RealDt < 0)
				RealDt = 0;

			int Timescale = std::clamp(Config()->m_SvAiTimescale, 1, 100);
			bool Turbo = Config()->m_SvAiTurbo != 0;
			int64_t TickDuration = time_freq() / TickSpeed();

			if(Turbo)
			{
				VirtualTimeAccumulator += TickDuration * 50;
			}
			else
			{
				VirtualTimeAccumulator += RealDt * Timescale;
			}

			if(VirtualTimeAccumulator > TickDuration * 100)
				VirtualTimeAccumulator = TickDuration * 100;
			...
			int MaxTicksThisIter = Turbo ? 50 : (Timescale * 2 + 5);
			int TicksRun = 0;
			while(VirtualTimeAccumulator >= TickDuration && TicksRun < MaxTicksThisIter)
			{
				VirtualTimeAccumulator -= TickDuration;
				TicksRun++;
			...
			if(NewTicks)
			{
				int64_t SnapNow = time_get();
				if(Timescale == 1 && !Turbo)
				{
					DoSnapshot();
				}
				else if(SnapNow - LastSnapWallTime >= time_freq() / 30)
				{
					LastSnapWallTime = SnapNow;
					DoSnapshot();
				}
			...
			else
			{
				...
				if(Turbo)
				{
					net_socket_read_wait(m_NetServer.Socket(), 0us);
				}
				else
				{
					int64_t RemainingVirtual = TickDuration - VirtualTimeAccumulator;
					if(RemainingVirtual > 0)
					{
						int64_t RealRemainingMicros = (RemainingVirtual * 1000000LL) / (time_freq() * Timescale);
						if(RealRemainingMicros > 0)
						{
							auto WaitDuration = std::chrono::microseconds(std::min<int64_t>(RealRemainingMicros, 20000));
							net_socket_read_wait(m_NetServer.Socket(), WaitDuration);
						}
					}
					else
					{
